...
while acao_feita != acao2:
    ...
    for profissoes_arara in conteudo_arara:    
        if not profissoes_faladas.count(profissoes_arara):
            ...
            lista_profissoes_fora = profissao_fora
    ...
    if len(conteudo_arara) != len(profissoes_faladas):
        ...
        if qtd_diferentes > 0:
            print(f'Poxa, Barbie! Você acabou desorganizando essa arara, e {qtd_diferentes} profissões vão ficar de fora da conversa. São elas: {lista_profissoes_fora}.')
